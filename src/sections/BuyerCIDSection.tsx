...
interface BuyerCIDSectionProps {
  value: string;
...
export function BuyerCIDSection({ value, onChange, onNext, onBack }: BuyerCIDSectionProps) {
  const [error, setError] = useState('');

  const handleNext = () => {
    if (!value.trim()) {
      setError('Please enter the buyer\'s CID number');
      return;
    }
    if (!/^\d{11}$/.test(value)) {
  ...
  return (
    <FormStepSection
      headline={['WHAT IS THE', 'BUYER\'S', 'CID NUMBER?']}
    >
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 text-sm text-[#6B7280] dark:text-gray-400">
            11-digit CID as shown on the buyer's Bhutan ID card.
          </p>
        </div>
