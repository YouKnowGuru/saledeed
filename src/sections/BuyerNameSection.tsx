...
interface BuyerNameSectionProps {
...
export function BuyerNameSection({ value, onChange, onNext, onBack }: BuyerNameSectionProps) {
  const [error, setError] = useState('');

  const handleNext = () => {
    if (!value.fullName.trim()) {
      setError('Please enter the buyer\'s full name');
      return;
    }
    setError('');
    onNext();
  ...
  return (
    <FormStepSection
      headline={['WHAT IS THE', 'BUYER\'S', 'FULL NAME?']}
    >
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 text-sm text-[#6B7280] dark:text-gray-400">
            Use the name as it appears on the buyer's Citizenship ID.
          </p>
        </div>
